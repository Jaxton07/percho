...
- **renderer 模块图必须无环**（改完用脚本自查）：有环时「模块体在谁的 import 过程中执行」不确定，模块级副作用会读到尚未求值的 store 绑定（曾因此把 `toggleInList` 拆成叶模块 `lib/toggle-in-list.ts`）。纯函数/常量不要挂在会反向 import 回来的模块上
...
| `scripts/check-sidebar-group-scroll.mjs` | 左栏分组会话列表「限高 248px + 组内独立滚动 + 边缘淡出」CDP 验收（spec §7 + 淡出）：结构/尺寸契约 + **真实 wheel** 的滚动归属（内层中部/上下边界/标题行/不溢出列表交给外层）+ 淡出状态机（静止在顶只下渐隐 / 中部两面 / 到底只上渐隐 / 不溢出列表无淡出）与「淡出条内仍可命中」；只用 `data-*` 属性与 DOM 相邻关系定位，退出码 0 通过 / 1 断言失败 / 2 环境不满足 |
...
| `src/main/window.ts` | BrowserWindow：**阻止页面内链接导航主窗口（防相对 Markdown 链接白屏），新窗口只允许 http(s) 外链进系统浏览器**；sandbox + preload；启动底色跟随主题防白闪（已解析主题经 `?theme=` query 传 renderer）；窗口框架按平台分流（mac hiddenInset / Win frameless+titleBarOverlay / Linux 原生）；**关窗语义按平台**：macOS 红点/⌘W = 隐藏窗口（`close` 事件 `preventDefault + hide()`，`index.ts` 的 `before-quit` 调 `markQuitting()` 放行 ⌘Q；`activate` 恢复 show+focus）；Windows 点 ✕ = 退出（微软惯例不变），**仅当渲染端接管了退出确认（`app:setQuitGuard`）时拦下弹确认窗**，并起 1.5s 兑底计时等 `app:quitDialogShown` 回执（超时/未接管/页面已销毁一律放行——见 PITFALLS）；`did-start-loading` 时清 guard（重载期不拦）；导出 `resolveTheme`/`applyChromeTheme`/`markQuitting`/`setQuitGuard`/`ackQuitDialog` |
...
| `lib/sidebar-groups.ts` | 左栏纯派生层（有单测）：`mergeSidebarSessions`（**会话目录 + 内存会话合并**：按 `sessionId` 去重、内存覆盖历史、历史保序；同 ID 走 `mergeSessionMeta`——运行态字段（name/model/active/messageCount）以内存为准，但 **`createdAt` 始终取历史权威值、内存 `modifiedAt` 缺失时保留历史值**，否则排序键会从 `modifiedAt` 掉到 `createdAt`、行在打开/卸载时伪移动；**存在性不靠它**：本进程经手过的会话在进内存时就已被写进目录，合并只补「还没落盘的新会话」与运行态）+ `primaryNavigationSessions` / `deriveSidebarNavigation`（**左栏生产装配，Sidebar 与单测走同一条路**：合并 → 按 `isPrimaryNavigationSession` 过滤只读子会话 → **同一份集合**同时喂给 `deriveProjects` 与 `deriveSidebarGroups`，否则只读子会话会凭空长出一个项目组）+ `deriveSidebarGroups`（合并结果 + 项目表 → 日常/项目分组、置顶排序、搜索过滤、展开态推断；**默认展开组只看显式 `activeCwd`（新会话页 = draft.cwd），不从 sessions 反查 active**）+ `toggleExpandedGroup`（第 4 参 `touched` 定起点：没操作过用派生层默认集，已操作过用用户记录——**空数组合法表示全部折叠**）。「项目」小标 v7 起不可折叠，不再有 `PROJECTS_GROUP_KEY` 这类专用 key |
| `lib/toggle-in-list.ts` | 置顶切换纯函数（叶模块，有单测）：会话/项目/手动添加项目共用；**必须留在叶模块**，挂到 `sidebar-groups` 上会重新形成 store 环（见硬约束）。`toggleExpandedGroup` 仍在 `sidebar-groups` |
...
| `stores/projects.ts` / `theme.ts` / `ui.ts` / `ui-preferences.ts` / `update.ts` / `ui-plugins.ts` / `toasts.ts` / `app-quit.ts` | 会话目录与项目表（**`allSessions` = 会话目录 = 上次全量对账的磁盘历史 ∪ 本进程经手过的会话**，是「左栏/顶栏行存在不存在」的**唯一来源**：`load` 只在进新会话页/启动时整表对账，有 **latest-wins** 单调 `loadSeq` 守卫（更晚的 load 一旦发起，旧请求的成功/失败都不写 `allSessions/loading/loaded`）；**写穿 `absorbOpenSessions`**：`useSessionsStore` 订阅（会话一进内存就补进目录，只补缺不覆盖）+ `load` 结尾再补一次（0 消息会话还没有会话文件，磁盘快照必然没有它）——内存策略(GC)卸载只撤运行态，行不会消失（2026-09-21 事故修复）；`deriveProjects`：手动添加的按时间倒排；`deriveSessions`：置顶分区在左 + 其余按最后活动倒序；`deleteSession` / `deleteProject` / `openSession`（已加载会话走 switchSession，否则 openFromHistory）/ `applySessionName`）/ 主题与背景（init 在 render 前 await 防闪烁）/ todo 面板展开 + diff 侧栏开关（内存态）/ **左栏偏好 `barSessionsVisible` / `sidebarCollapsed` / `expandedGroups` + `expandedGroupsTouched`（成对语义：false = 从没手动开合过 → 走默认推断，true = 完全以 `expandedGroups` 为准，**空数组 = 全部折叠**；旧文件缺字段时按记录是否非空推断）/ `pinnedProjects` + 中央动画开关 + 置顶会话 `pinnedSessions`（v8 起它同时就是顶栏胶囊内容与顺序，拖动排序走 `reorderPinned`）+ **`sessionPermissionModes`（D7：按会话记住的权限模式，只存非 default；`rememberPermissionMode` / `forgetPermissionMode`）** + 上次项目目录 `lastCwd`（App 启动预填 cwd，只带目录不恢复会话）**（均持久化 ui-state；删会话时 unpin 清理）/ 更新态 / UI 插件面板 / 全局 Toast（顶栏右侧，非阻塞自动消失；扩展 notify 走 pushExtension：同源同文 8s 去重 + 可见栈 3 溢出折叠） |
