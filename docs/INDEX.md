...
| `stores/drafts.ts` | 草稿（文本/图片/slash 胶囊/@ 引用 attachments）按会话持久 + `COMPOSER_FOCUS_EVENT`（撤回回填后聚焦输入框） |
...
| 会话分叉 / 撤回 | backend `forkSession`/`recallMessage`（目标解析在 session/messages.ts；**agent 运行或压缩期间均拒绝**）；renderer `stores/sessions.ts`（fork 新 tab 打开；recall 草稿回填 + COMPOSER_FOCUS_EVENT）+ `chat/message-actions.tsx`（ForkButton 挂轮次末段正文/RecallButton 挂用户气泡，运行/压缩期间禁用） |
