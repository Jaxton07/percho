...
| 顶栏显隐 | 设置项「显示顶栏」（`settings/AppearancePanel.tsx` → `ui-preferences.ts`）→ `App.tsx` 条件渲染 `SessionTabBar`；顶栏关闭时兜底：左栏头 44px 拖拽带 + 「＋」、聊天列顶 12px 隐形拖拽带、左栏底部「本轮改动」 |
