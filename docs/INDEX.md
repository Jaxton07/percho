...
| `lib/session-gc.ts` | 会话内存驻留策略（**纯函数，有单测**）：`pickUnloadCandidates`（保护/草稿/活跃/刚用过 → 剔除；剩余按最久未用排序，保留 K 个，超的与晾过 `idleTimeoutMs` 的一并作候选）+ `isProtected`（保护条件唯一事实源：draft / 0 消息 / 权限模式非 default / agentActive / 待审批 / 待应答 / 未读绿点 / 压缩中 / 排队跟发）+ `GC_DEFAULTS`（K=3、fresh 10s、idle 5min）。不 import React / 不调 IPC |
| `lib/use-session-gc.ts` | 内存策略接线层：挂载跑一次 + 60s 兜底 tick + 只订阅 `sessions`/`activeSessionId`（**不订阅 `transcript.bySession`**，它每 token 都变）+ 卸载前逐个再确认（活跃/已关就跳过）+ dev 打 `[session-gc] unload <id> reason=...`；K=3 常量在这里 |
