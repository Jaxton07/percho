...
| `scripts/check-sidebar-group-scroll.mjs` | 左栏分组会话列表「限高 248px + 组内独立滚动」CDP 验收（spec §7）：结构/尺寸契约 + **真实 wheel** 的滚动归属（内层中部/上下边界/标题行/不溢出列表交给外层）；只用 `data-*` 属性与 DOM 相邻关系定位，退出码 0 通过 / 1 断言失败 / 2 环境不满足 |
