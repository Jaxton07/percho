...
| 长会话渲染性能（切会话卡顿 / 首屏挂载量） | 窗口策略 `chat/mount-window.ts`（纯函数 + 测试；常量与滚动补偿在 `chat/MessageList.tsx`——补挂用旧首行 `offsetTop` 补偿 scrollTop，容器 `[overflow-anchor:none]`）；工具卡溢出测量调度器在 `chat/ToolCallCard.tsx`（共享测量队列 + 共享 ResizeObserver）。背景与实测数字见 PITFALLS「长会话切会话卡顿」 |
