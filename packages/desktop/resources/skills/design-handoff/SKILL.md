...
1. 读 `.local/docs/INDEX.md`（项目索引 + 「想改 X 改哪里」速查表）和相关代码。
...
**不要把 spec/plan 文件写进 `.local/docs/INDEX.md`**——agent 协作产物随任务生灭、会不定期清理，索引只标 `.local/agent-work/` 目录一行，不标其下的 spec/plan/channel 单个文件。HANDOFF.md 里有完整路径，可发现性靠频道目录本身。
