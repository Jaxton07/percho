...
6. **沟通协议**（见下）
7. **自动唤醒提示**：写明实施会话开工时应订阅频道（channel_subscribe 工具，主题 = 目录名）；订阅后你写 channel 文件会自动唤醒对方查收，无需用户人工传话
...
- **写 channel 文件 = 通知对方**：合并写入（一次做完一组修改再写）；纯排版/无信息量变更不要写——避免无谓唤醒甚至乒乓
...
交付后（用户开了实施会话后）：你的 channel 文件写入会自动唤醒实施会话查收——需要传达新信息时**直接写文件即可**（IMPL-NOTES 回应、REVIEW 意见），不必等用户传话；但同样遵守「无信息量不写」。
...
- 频道文件高频碎写 / 无信息量写入——每次写入都唤醒对方，乒乓就是这么烧起来的
